...
public partial class SpectreLayoutDisplayService : IDisplayService
{
    ...
    private void RenderMapPanel(Room currentRoom) =>
        UpdateMapPanel(MapRenderer.BuildMarkupMap(currentRoom, _currentFloor));
    ...
    /// <summary>
    /// Builds the Spectre markup string for the Gear panel from the player's equipped items.
    /// Extracted for testability — mirrors <see cref="BuildPlayerStatsPanelMarkup"/>.
    /// </summary>
    /// <param name="player">The player whose equipped items are rendered.</param>
    /// <returns>Trimmed Spectre markup string ready for use in a <see cref="Panel"/>.</returns>
    ...
    {
        var panel = new Panel(new Markup(BuildGearPanelMarkup(player)))
